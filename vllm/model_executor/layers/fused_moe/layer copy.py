...
import torch
import csv
import numpy as np # Used for converting torch tensor to lists for CSV
...
class FusedMoE(CustomOp):
    ...
    def select_experts(
        ...
        # DeepSeekv2 uses grouped_top_k
            ...
            #Nirmal: Save topk_ids to CSV for analysis
            # --- START TOKEN ROUTING LOGGING ---
            
            # The layer name (e.g., 'model.layers.2.mlp') is used as the unique ID
            layer_name = self.layer_name
            dp_rank = int(os.environ.get("VLLM_DP_RANK", 0))
            
            # Transfer data to CPU and convert to numpy array/list for logging
            expert_ids_on_cpu = topk_ids.cpu().numpy() # shape [num_tokens, top_k]
            expert_weights_on_cpu = topk_weights.cpu().numpy() # shape [num_tokens, top_k]

            num_tokens_in_batch = expert_ids_on_cpu.shape[0]
            top_k = self.top_k

            # Use /tmp for shared, temporary storage.
            output_file = f"/tmp/vllm_moe_routing_rank_{dp_rank}.csv"
            ...
            with open(output_file, 'a', newline='') as f:
                ...
                if not file_exists:
                    header = ["layer_name", "dp_rank", "batch_token_index"]
                    for k in range(top_k):
                        header.append(f"expert_id_k{k}")
                    for k in range(top_k):
                        header.append(f"expert_weight_k{k}")
                    writer.writerow(header)

                # Write data row by row
                for batch_token_idx in range(num_tokens_in_batch):
                    # Batch token index (local to this chunk/batch) serves as your simple token ID
                    row = [layer_name, dp_rank, batch_token_idx]
                    
                    # Add expert IDs (e.g., [1, 5])
                    row.extend(expert_ids_on_cpu[batch_token_idx].tolist())
                    
                    # Add expert weights (e.g., [0.6, 0.4])
                    row.extend(expert_weights_on_cpu[batch_token_idx].tolist())
                    
                    writer.writerow(row)
            
            # --- END TOKEN ROUTING LOGGING ---
            
            # IMPORTANT: Re-add the code that was stripped to access `token_expert_indices` 
            # if that tensor is needed later in the original vLLM code.
            # (In this example, it seems to be unused, but check your version).
            # The function must still return: topk_weights, topk_ids, zero_expert_result
