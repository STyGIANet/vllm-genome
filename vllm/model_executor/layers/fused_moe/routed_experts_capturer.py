...
class RoutedExpertsCapturer:
    ...
    def get_captured_experts_for_ranges(
        ...
        for start, end in ranges:
            if end <= start:
                continue
            parts.append(self.device_buffer[start:end].cpu().numpy())
    ...
    def get_unique_layer_expert_pairs_for_ranges(
        ...
        num_layers = self.device_buffer.shape[1]
        top_k = self.device_buffer.shape[2]
        ...
        for start, end in ranges:
            ...
            token_slice = self.device_buffer[start:end]
...
_global_expert_capturer: RoutedExpertsCapturer | None = None


def set_global_experts_capturer(capturer: RoutedExpertsCapturer | None) -> None:
