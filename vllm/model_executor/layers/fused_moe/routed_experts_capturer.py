...
class _RoutedExpertsCapturerReal(RoutedExpertsCapturer):
    ...
    def get_captured_experts(self, num_tokens: int) -> np.ndarray | None:
        if self.host_cache is None or num_tokens <= 0:
            return None
        return (
            self.device_cache.buffer[:, :num_tokens, :]
            .permute(1, 0, 2)
            .cpu()
            .numpy()
        )
    ...
    def get_captured_experts_for_ranges(
        ...
        if self.host_cache is None or not ranges:
            return None
        ...
        for start, end in ranges:
            if end <= start:
                continue
            parts.append(
                self.device_cache.buffer[:, start:end, :]
                .permute(1, 0, 2)
                .cpu()
                .numpy()
            )
    ...
    def get_unique_layer_expert_pairs_for_ranges(
        ...
        num_layers = self.device_cache.buffer.shape[0]
        top_k = self.device_cache.buffer.shape[2]
        layer_ids = torch.arange(
            num_layers,
            dtype=torch.int32,
            device=self.device_cache.buffer.device,
        ...
        for start, end in ranges:
            ...
            token_slice = self.device_cache.buffer[:, start:end, :].permute(1, 0, 2)
